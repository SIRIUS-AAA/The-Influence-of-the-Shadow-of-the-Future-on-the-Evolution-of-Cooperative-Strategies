...
目标：
- 在 continuation（未知轮）中，让每一轮的继续概率 p_t 按轨迹（上升/下降/随机游走）变化
- 停机机制保持几何停机：进入第 t 轮由上一轮的 p_{t-1} 判定
- 保留交互式与批量（grid）两种运行模式
- 逐轮写 CSV：含 path、p_now 等
- 可选输出按 p 分箱的汇总：合作/协调率与平均收益
...
def get_action(player_func: Callable, prompt_base: str, summary: str, p_now: float, round_i: int, mode: str) -> str:
    if mode == "baseline":
        ...
        prompt = (
            prompt_base +
            f"The continuation probability this round is p={p_now:.2f}.\n" +
            summary +
...
def run_match_dynamic(
    *,
    ...
    for i, p_now in enumerate(p_seq, start=1):
        # 是否进入第 i 轮：取决于上一轮的 p_{i-1}
        if i > 1 and (random.random() > p_seq[i-2]):
            ended_by = "prob_stop"
            break
        if i == max_round:
            ended_by = "cap"
        ...
        base1 = p1_base + (f"The continuation probability this round is p={p_now:.2f}.\n" if p_in_prompt else "")
        base2 = p2_base + (f"The continuation probability this round is p={p_now:.2f}.\n" if p_in_prompt else "")
...
def main():
    print("\n=== Experiment 3 — Continuation with Dynamic p_t (交互式/批量) ===")
    ...
    p_in_prompt = ask_yes_no("是否把当前轮 p 写入提示？（建议 Yes）", default_yes=True)
